...
function Header() {
  ...
  return (
    <header className="w-full bg-[#0C2B4E] shadow px-4 py-3 flex items-center gap-3">
      ...
      <form
        role="search"
        className="hidden sm:flex items-center gap-2 mx-4 flex-1 max-w-xl"
        onSubmit={(e) => e.preventDefault()}
      >
        <input
          type="search"
          placeholder="Search for anything"
          className="w-full px-3 py-2 rounded-md text-sm outline-none bg-[#1A3D64] text-white focus:ring-2 focus:ring-[#4a90e2] placeholder:text-gray-200"
        />
        <button
          type="submit"
          className="px-3 py-2 rounded-md bg-[#163d6b] text-white text-sm hover:bg-[#1f4f88] transition cursor-pointer"
        >
          ⌕
        </button>
      </form>
      ...
      <div className="hidden sm:flex items-center gap-3 ml-auto">
        <button className="px-3 md:px-4 py-1.5 rounded-md bg-[#1A3D64] text-white text-xs md:text-sm font-semibold shadow-sm hover:bg-[#f8c549] transition whitespace-nowrap">
          Schedule Session
        </button>
