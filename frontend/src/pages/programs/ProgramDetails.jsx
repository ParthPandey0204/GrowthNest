...
function ProgramDetails() {
  const { id } = useParams();
  const { data: program, isLoading, error } = useCourse(id);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.4fr_0.8fr]">
        <section className="rounded-3xl border border-slate-200 bg-white p-6"><h2 className="text-xl font-semibold text-slate-900">Lessons</h2><div className="mt-4 space-y-3">{program.lessons?.length ? program.lessons.map((lesson, index) => <div key={lesson.id} className="rounded-2xl bg-slate-50 px-4 py-3"><p className="font-medium text-slate-800">{index + 1}. {lesson.title}</p><p className="mt-1 text-xs uppercase tracking-wide text-slate-500">{lesson.type}</p></div>) : <p className="text-sm text-slate-500">Lessons will be published soon.</p>}</div></section>
